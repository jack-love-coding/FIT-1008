...
class QuadraticProbeTable:
    ...
    """
    A class that implement a complete version of a hash table using Quadratic Probing to resolve collisions
    """

    def __init__(self, size = 10000):
        """
        Construction function that initialize an instance of class LinearProbeTable
        :parameter: b: b for universal hashing
        :parameter: size: the size of this hash table
        """
    ...
    def __getitem__(self, key):
        """
        To get item in the QuadraticProbeTable instance according to the given key.
    ...
    def __setitem__(self, key, data):
        ...
        for _ in range(self.table_size):
            if self.array[position] is None:
                ...
                position = (base_position + (_ + 1) ** 2) % self.table_size # use quadratic probing to resolve collisions
    ...
    for file in lst_file:
        for b in lst_b:
            for size in lst_table_size:
                if (b+1) != size:
                    class QuadraticProbeTable_with_b(QuadraticProbeTable):
                        def __init__(self,size):
                            super(QuadraticProbeTable_with_b, self).__init__(size)
                            self.b = b
                    # test time consumed for each combination of the values for table size and b
                    inputfile = open(file,'r')
                    hash_table = QuadraticProbeTable_with_b(size)
