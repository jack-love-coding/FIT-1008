...
class SeparateChainingTable:
    """
    A class that implement a complete version of a hash table using Separate chaining(BST) to resolve collisions
    """

    def __init__(self, size=10000):
        """
        Construction function that initialize an instance of class SeparateChainingTable
        """
    ...
    def __getitem__(self, key):
        """
        To get item in the SeparateChainingTable instance according to the given key.
    ...
    def __contains__(self, item):
        ...
        for i in range(self.table_size):
            if self.array[i] is not None:
                if self.array[i].__contains__(item):
                    return True
        return False
    ...
    for file in lst_file:
        for b in lst_b:
            for size in lst_table_size:
                if (b+1) != size:
                    class SeparateChainingTable_with_b(SeparateChainingTable):
                        def __init__(self,size):
                            super(SeparateChainingTable_with_b, self).__init__(size)
                            self.b = b
                    # test time consumed for each combination of the values for table size and b
                    inputfile = open(file,'r')
                    hash_table = SeparateChainingTable_with_b(size)
